...
public class Assignment {
    private ArrayList<AssignmentStages> Stages = new ArrayList<>();
    ...
    public Assignment(GregorianCalendar dueDate, Time dueTime, String name) {
        this.curStage = this.Stages.get(0);
        this.dueDate = dueDate;
        this.name = name;
        this.dueTime=dueTime;
    }
    public void addStage(AssignmentStages stage){
        Stages.add(stage);
    }
    ...
    public void dueMsg(){
        long min = dueDate.getTimeInMillis()+((long)dueTime.getHours()*60*1000*60)+
                ((long)dueTime.getMinutes()*60*1000);
        Date date = new Date(min);
        Timer t = new Timer();
        t.schedule(new Reminder(),date);
    }
